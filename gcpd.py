...
async def download_segment(
    session, ts_url, tmpdir, idx, overall_progress, semaphore, count_segments=False
):
        ...
        try:
            ...
            if process.returncode == 0:
                ...
                if retry_count < max_retries:
                    ...
                    print(
                        "Достигнуто максимальное количество попыток. Конвертация не удалась."
                    )
                    return False
...
async def main(url, result_file, no_pre_download):
    async with aiohttp.ClientSession() as session:
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            with tqdm(
                total=None, desc="Основной плейлист", unit="B", unit_scale=True
            ) as pbar:
            ...
            if ts_or_bin_pattern.search(main_playlist_content):
                ...
                if not re.match(r"^https?://", tail):
                    print(
                        "В содержимом заданной ссылки нет прямых ссылок на файлы *.bin (*.ts) (первый вариант),"
                    )
                    print(
                        "также последняя строка в ней не содержит ссылки на другой плей-лист (второй вариант)."
                    )
                    print(
                        "Либо указана неправильная ссылка, либо GetCourse изменил алгоритмы."
                    )
                    print(
                        "Если уверены, что дело в изменившихся алгоритмах GetCourse, опишите проблему здесь:"
                    )
                    print(
                        "https://github.com/mikhailnov/getcourse-video-downloader/issues (на русском)."
                    )
                    print(
                        "Если уверены, что это ошибка скрипта, то опишите проблему здесь:"
                    )
                    print(
                ...
                with tqdm(
                    total=None, desc="Вторичный плейлист", unit="B", unit_scale=True
                ) as pbar:
            ...
            if no_pre_download:
                overall_pbar = tqdm(
                    total=len(ts_urls), desc="Общий прогресс", unit="сегмент"
                )
            else:
                overall_pbar = tqdm(
                    total=total_size, desc="Общий прогресс", unit="B", unit_scale=True
                )
            ...
            tasks = [
                download_segment(
                    ...
                    overall_pbar,
                    semaphore,
                    count_segments=no_pre_download,
                )
...
def starter():
    ...
    parser.add_argument(
        ...
        "--no-pre-download",
        action="store_true",
        help="Пропустить предварительную загрузку размеров",
    )
    parser.add_argument(
        "-a",
        action="store_true",
        help="Automaticaly process all .har files in current dir",
    )
        ...
        for filename in os.listdir("."):
            if filename.endswith(".har"):
                ...
                for count, url in enumerate(har_parser(filename), start=1):
                    url = str(url)
                    result_file = (
                        f"{os.path.splitext(os.path.basename(filename))[0]}{count}"
                    )
                    print("url =", url)
                    print("result_file =", result_file)
                    asyncio.run(main(url, result_file, args.no_pre_download))
